...
class JobScheduler:
    ...
    async def _execute_job_wrapper(self, job_search_id: int):
        """Wrapper to execute job with database session"""
        db = SessionLocal()
        try:
            service = JobScraperService(db)
            await service.execute_job_search(job_search_id)
        except Exception as e:
            logger.error(f"Error executing scheduled job {job_search_id}: {e}", exc_info=True)
        finally:
            db.close()
