...
class JobFilter:
    """Filter jobs based on configured criteria - supports LinkedIn, Indeed, and Fantastic Jobs"""
    ...
    def _should_filter_fantastic_jobs(self, job: Dict) -> Tuple[bool, Optional[str]]:
        """Filter logic for Fantastic Jobs career site scraper"""
        # Check company name exclusions (field is "organization")
        company_name = job.get("organization", "")
        if self._matches_exclusion_pattern(company_name, self.company_name_excludes):
            return True, f"Company name matches exclusion: {company_name}"
        
        # Check industry exclusions (field is "linkedin_org_industry")
        industry = job.get("linkedin_org_industry", "")
        if self._matches_exclusion_pattern(industry, self.industries_excludes):
            return True, f"Industry matches exclusion: {industry}"
        
        # Check employee count range (field is "linkedin_org_employees")
        ...
        if employee_count is not None:
            try:
                ...
                # Can't parse employee count, skip this filter
                pass
        
        # Check job description filters if provided
        if self.job_description_filters:
            job_desc = self._clean_html(job.get("description_text", ""))
            if not self._matches_regex(job_desc, self.job_description_filters):
                return True, "Job description doesn't match required filters"
    ...
    def should_filter_out(self, job: Dict) -> Tuple[bool, Optional[str]]:
        ...
        elif self.platform == "fantastic_jobs":
            return self._should_filter_fantastic_jobs(job)
    ...
    def filter_jobs(self, jobs: List[Dict]) -> Tuple[List[Dict], List[Dict]]:
        ...
        for job in jobs:
            ...
            if should_filter:
                ...
                filtered.append(job)
                # Get job title based on platform
                title = job.get('title') or job.get('displayTitle') or 'Unknown'
                logger.debug(f"Filtered {self.platform} job '{title}': {reason}")
            else:
                passed.append(job)
