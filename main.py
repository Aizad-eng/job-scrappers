...
app = FastAPI(
    ...
    description="API for scraping LinkedIn, Indeed, and Fantastic Jobs career sites",
    version="2.0.0"
)
...
async def create_job_search(
    ...
    logger.info(f"Created job search: {db_job_search.name} (ID: {db_job_search.id}, Platform: {db_job_search.platform})")
...
async def list_job_searches(
    ...
    if platform:
        query = query.filter(JobSearch.platform == platform)
...
async def duplicate_job_search(job_search_id: int, db: Session = Depends(get_db)):
    ...
    # Create copy with new name - include all fields including Fantastic Jobs ones
    new_job_search = JobSearch(
        ...
        title_search=original.title_search,
        location_search=original.location_search,
        employer_search=original.employer_search,
        time_range=original.time_range,
        include_ai=original.include_ai,
        include_linkedin=original.include_linkedin,
        # Filtering
...
async def trigger_job(request: TriggerJobRequest, db: Session = Depends(get_db)):
    ...
    return {
        "message": f"Job search '{job_search.name}' ({job_search.platform}) triggered successfully",
...
async def health_check():
    ...
    return {
        ...
        "supported_platforms": ["linkedin", "indeed", "fantastic_jobs"]
    }
